...
let rselect rs ar = Array.get ar (Random.State.int rs (Array.length ar))

module Check_type = struct
  type t = Temp | Free_space | Packet_drops [@@deriving sexp, enumerate]
...
let random_host_set rs n =
  let rec loop set =
    if Set.length set = n then set else loop (Set.add set (random_host rs))
  in
  loop Host.Name.Set.empty

module State = struct
  type t = {
    hosts : Host.Name.Set.t;
    active :
      (Host.Info.t * (Time_float_unix.t * Check.Outcome.t) Check.Name.Map.t)
      Host.Name.Map.t;
  ...
  let activate t rs host =
    let info : Host.Info.t =
      { name = host; address = random_ip rs; boot_time = t.time }
    in
    let active =
      Map.set t.active ~key:host ~data:(info, Check.Name.Map.empty)
    in
    ({ t with active }, info)

  let activate_pct t rs (pct : float) =
    Set.fold t.hosts ~init:t ~f:(fun t host ->
        if Float.( < ) (Random.State.float rs 1.) pct then
          fst (activate t rs host)
        else t)

  let create rs time ~num_hosts ~pct_initially_active =
    let hosts = random_host_set rs num_hosts in
    activate_pct
      { hosts; active = Host.Name.Map.empty; time }
      rs pct_initially_active
  ...
  let chooser rs options =
    ...
    let total_weight =
      List.map ~f:fst options |> List.fold ~init:0. ~f:( +. )
    in
    ...
    fun () ->
      let rec find i x_remaining =
        if i = Array.length choices - 1 then snd choices.(i)
          ...
          if Float.( <= ) x_remaining current_prob then snd choices.(i)
  ...
  let next_event t ~time_scale rs =
    ...
    let choose_check =
      chooser rs (equiprobable (Check_type.all : Check_type.t list))
    in
    let update_type =
      chooser rs [ (20, Activate_host); (90, Check_success); (8, Check_fail) ]
    in
    fun t ->
      let time_delta =
        Random.State.float rs (Time_float_unix.Span.to_sec time_scale)
        |> Time_float_unix.Span.of_sec
      in
      let t = { t with time = Time_float_unix.add t.time time_delta } in
      let change_check outcome =
        ...
        | Some (info, checks) ->
            ...
            in
            let checks =
              Map.set checks ~key:check ~data:(when_checked, outcome)
            in
            let active = Map.set t.active ~key:host ~data:(info, checks) in
            let report =
              [
                Event.create t.time
                  (Check (Report { host; check; when_checked; outcome }));
...
let stream rs time ~num_hosts ~pct_initially_active ~time_scale =
  let sequence =
    ref (sequence rs time ~num_hosts ~pct_initially_active ~time_scale)
  in
  stage (fun () ->
