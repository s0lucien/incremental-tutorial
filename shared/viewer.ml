...
let update_interval = Time_float_unix.Span.of_sec 0.3
...
type 'a t = {
  ...
  compute_times : Time_float_unix.Span.t Queue.t;
}

let get_and_clear_avg_compute_time t =
  let sum = Queue.sum (module Time_float_unix.Span) ~f:Fn.id t.compute_times in
  let length = Queue.length t.compute_times in
  let rval = Time_float_unix.Span.scale sum (1. /. Float.of_int length) in
...
let create ~print =
  ...
  Clock.every' update_interval (fun () ->
      ...
      | Some data ->
          ...
          Core.printf
            !"Total events seen by viewer: %d\t\tper sec %.3f\t\tavg time: %s\n\n\
              %!"
            t.total_events
            (Float.of_int t.total_events /. diff)
            (Time_float_unix.Span.to_string avg_compute_time);
          printf "------------------------------\n\n%!";
          print data;
          t.last_refresh <- now);
  t
...
let compute t f =
  ...
  let rval = f () in
  let stop = Time_float_unix.now () in
  Queue.enqueue t.compute_times (Time_float_unix.diff stop start);
  rval
