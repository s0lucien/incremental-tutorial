...
module Incremental = struct
  ...
  let flatten_maps (mm : State.t Incr.t) :
    ...
    diff_map (mm >>| State.hosts) ~f:(fun ~old input ->
        match old with
        | None ->
            Map.fold input ~init:Map.Poly.empty
              ~f:(fun ~key:key1 ~data:(_, data) acc ->
                Map.fold data ~init:acc ~f:(fun ~key:key2 ~data acc ->
                    Map.set acc ~key:(key1, key2) ~data))
        | Some (old_input, old_output) ->
            let changes =
              Sequence.bind
                ...
                ~f:(function
                | key1, `Left (_, m) ->
                    Sequence.map (Map.to_sequence m) ~f:(fun (key2, _) ->
                        `Remove (key1, key2))
                | key1, `Right (_, m) ->
                    Sequence.map (Map.to_sequence m) ~f:(fun (key2, data) ->
                        `Add ((key1, key2), data))
                | key1, `Unequal ((_, m1), (_, m2)) ->
                    Map.symmetric_diff
                      ~data_equal:(fun (t1, _) (t2, _) ->
                        Time_float_unix.equal t1 t2)
                      m1 m2
                    |> Sequence.bind ~f:(function
                         | key2, `Left _ ->
                             Sequence.singleton (`Remove (key1, key2))
                         | key2, `Right d ->
                             Sequence.singleton (`Add ((key1, key2), d))
                         | key2, `Unequal (_, d2) ->
                             Sequence.of_list
                               [ `Remove (key1, key2); `Add ((key1, key2), d2) ]))
            in
            Sequence.fold changes ~init:old_output ~f:(fun acc change ->
                match change with
                | `Add (key, data) -> Map.set acc ~key ~data
